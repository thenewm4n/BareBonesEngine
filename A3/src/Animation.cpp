...
Animation::Animation(const std::string& animationName, const sf::Texture& tex, size_t frameCount, size_t duration)
    : m_name(animationName), m_sprite(tex), m_frameCount(frameCount), m_duration(duration), m_currentFrame(0)
{
    m_size = Vec2(static_cast<float>(tex.getSize().x) / frameCount, static_cast<float>(tex.getSize().y));
